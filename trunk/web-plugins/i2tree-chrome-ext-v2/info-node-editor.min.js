...
["\u00dd","?","?","?","?"]],removeSign=function(a){for(var b=1;b<vietnameseSigns.length;b++)for(var c=0;c<vietnameseSigns[b].length;c++)a=a.replace(vietnameseSigns[b][c],vietnameseSigns[0][b-1]);return a};String.prototype.allTrim=String.prototype.allTrim||function(){return this.split(/\-+/).join("-")};var safeStringForName=function(a){return removeSign(a).replace(/[^a-z0-9]/gi,"-").allTrim().toLowerCase()};
jQuery(function(){jQuery("#btn_save2dropbox").button().click(addInfoNode);jQuery("#selected_content").cleditor({width:"99%",height:"560px","margin-top":"10px"})});